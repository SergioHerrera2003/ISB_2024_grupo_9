#PLOTEO SEÑAL EEG - Respondiendo preguntas 12s
...
f = open("respondiendo_preguntas_12seg.txt","r")
...
array = np.genfromtxt("./respondiendo_preguntas_12seg.txt", skip_header = 2)
...
plt.grid(linestyle=":")
plt.title("EEG - Señal obtenida al resolver preguntas ")
plt.xlabel("Tiempo (s)")
...
plt.grid(linestyle=":")
plt.title("EEG(uV)- Señal obtenida al resolver preguntas ")
plt.xlabel("Tiempo (s)")
plt.ylabel("EEG(uV)")
plt.legend(loc="upper right")
plt.xlim([0,140])
